...
mat4_t LOCAL translate(const vec3_t& v) {
...
void LOCAL translate(const vec3_t& v, mat4_t& trans, mat4_t& inv) {
    trans = translate(v);
    inv   = translate(-v);
}

mat4_t LOCAL scale(const vec3_t& v) {
...
void LOCAL scale(const vec3_t&v, mat4_t& trans, mat4_t& inv) {
    trans = scale(v);
    inv   = scale(vec3(recp(v.x), recp(v.y), recp(v.z)));
}

mat4_t LOCAL perspective(float angle_of_view, float aspect_ratio, float z_near, float z_far) {
