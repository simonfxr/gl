...
struct RefBase {
protected:
    T *_ptr; // fat pointer, saves one indirection
    atomic::Counter *_cnt;
...
protected:
    ...
    void set(const RefBase<T, C, R>& ref) {
...
public:
    ...
    bool valid() const { return refCount() > 0; }
...
private:
    RefBase<T, C, R>& operator =(const RefBase<T, C, R>& _);
};
...
struct Ref : public priv::RefBase<T, priv::RefCnt, Ref<T> > {
    ...
    Ref<T>& operator =(const Ref<T>& ref) { set(ref); return *this; }
...
struct WeakRef : public priv::RefBase<T, priv::WeakRefCnt, WeakRef<T> > {
    ...
    WeakRef<T>& operator =(const WeakRef<T>& ref) { set(ref); return *this; }
