...
struct Atomic
{
    ...
    T getAndAdd(T v) { return __sync_fetch_and_add(&value, v); }

    T getAndInc() { return getAndAdd(1); }

    T addAndGet(T v) { return __sync_add_and_fetch(&value, v); }

    T incAndGet() { return addAndGet(1); }

    T getAndSub(T v) { return __sync_fetch_and_sub(&value, v); }

    T getAndDec() { return getAndSub(1); }

    T subAndGet(T v) { return __sync_sub_and_fetch(&value, v); }

    T decAndGet() { return subAndGet(1); }
