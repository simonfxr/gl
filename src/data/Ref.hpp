...
struct AtomicCounter
{
    ...
    AtomicCounter(int32 initial) : count(initial) {}

    ~AtomicCounter() { ON_DEBUG(count.set(MARK)); }

    int32 decAndGet()
    {
        DEBUG_ASSERT(count.get() != MARK);
        return count.decAndGet();
    }
    void inc()
    {
        DEBUG_ASSERT(count.get() != MARK);
    ...
    {
        int32 val = count.get();
        DEBUG_ASSERT(val != MARK);
        return val;
    }
    bool xchg(int32 expected, int32 *val) { return count.xchg(expected, val); }
};
#endif
...
struct SeqCounter
{
    ...
    int32 decAndGet()
    {
        DEBUG_ASSERT(count != MARK && count > 0);
        return --count;
    }
    void inc()
    {
...
struct StrongCount
{
    ...
    static ref_count *make() { return new ref_count(1, 1); }
};
...
public:
    ...
    WeakRef<T, C> weak() { return WeakRef<T, C>(*this); }

    template<typename U>
    typename std::enable_if<std::is_base_of<T, U>::value, Ref<U, C> &>::type
    cast()
    {
        return reinterpret_cast<Ref<U, C> &>(*this);
    }
    ...
    cast() const
    {
        return reinterpret_cast<const Ref<U, C> &>(*this);
    }
};
...
public:
    ...
    template<typename U>
    typename std::enable_if<std::is_base_of<T, U>::value, WeakRef<U, C> &>::type
    cast()
    {
