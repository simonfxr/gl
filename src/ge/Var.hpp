...
struct Var EXPLICIT : public Command
{
    ...
    void interactive(const Event<CommandEvent> &,
...
public:
    GenVar(T *v, const std::string &name, const std::string &desc)
      : Var(VAR_GETSET, impl::Meta<T>::type(), name, desc), var(v)
    {}
    const T &value() const { return *var; }
    T &value() { return *var; }
    void get(CommandArg *val) override { Meta<T>::get(value(), val); }
...
public:
    GenVar(T *v, const std::string &name, const std::string &desc)
      : Var(VAR_GET, impl::Meta<T>::type(), name, desc), var(v)
    {}
    const T &value() const { return *var; }
    void get(CommandArg *val) override { Meta<T>::get(value(), val); }
}
...
namespace impl
{
    ...
    {
        static CommandParam type() { return NumberParam; }
    ...
    struct Meta<bool>
    {
        static CommandParam type() { return IntegerParam; }
