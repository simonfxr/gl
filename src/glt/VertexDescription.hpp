...
template <typename T>
struct GLType { typedef typename T::gl type; };

template <>
struct GLType<math::real> { typedef float type; };

template <>
struct GLType<color> { typedef color type; };
...
struct VertexDescription {
    typedef void (*store_function)(unsigned char *dest, const unsigned char *src);
...
#define VD_GL_CONSTR_V(name, ...) __VA_ARGS__ {}
...
#define VD_GL_ATTRS_DECL_V(name, size) static const ::glt::VertexAttr vertex_attrs[size];
#define VD_GL_ATTRS_DECL_F(t, n, size) 1 + size
#define VD_GL_ATTRS_DECL_Z(t, n) 1

#define VD_GL_DESC_DECL_V(name, size) static const ::glt::VertexDescription<name> desc;
