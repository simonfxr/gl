...
void Anim::renderScene(const ge::Event<ge::RenderEvent>& ev) {
    ...
    p2.position = vec2(simulation.x1 + simulation.l * sin_phi, - simulation.l * cos_phi);
...
void Sim::integrate(real dt) {
    ...
    real a2_x1 = acc_x1(x1 + dx1_dt * dt2, dx1_dt + a1_x1 * dt2,
                        phi + dphi_dt * dt2, dphi_dt + a1_phi * dt2);
    real a2_phi = acc_phi(x1 + dx1_dt * dt2, dx1_dt + a1_x1 * dt2,
                        phi + dphi_dt * dt2, dphi_dt + a1_phi * dt2);

    real a3_x1 = acc_x1(x1 + dx1_dt * dt2, dx1_dt + a2_x1 * dt2,
                        phi + dphi_dt * dt2, dphi_dt + a2_phi * dt2);
    real a3_phi = acc_phi(x1 + dx1_dt * dt2, dx1_dt + a2_x1 * dt2,
                          phi + dphi_dt * dt2, dphi_dt + a2_phi * dt2);

#define P2(a, b, c, dt) (a + (b + 0.5f * c * dt) * dt)

    real a4_x1 = acc_x1(P2(x1, dx1_dt, a3_x1, dt), dx1_dt + a3_x1 * dt,
                        P2(phi, dphi_dt, a3_phi, dt), dphi_dt + a3_phi * dt);
    real a4_phi = acc_phi(P2(x1, dx1_dt, a3_x1, dt), dx1_dt + a3_x1 * dt,
                          P2(phi, dphi_dt, a3_phi, dt), dphi_dt + a3_phi * dt);
...
void Anim::simulate(const ge::Event<ge::AnimationEvent>& ev) {
